...
# 훈련에 사용할 입력, 출력 값을 임의로 생성.
X = np.linspace(2, 10, num=50) # 2에서 10까지 숫자 50개 생성
Y = np.random.rand(50)*10 + 2  # 50개의 난수를 생성해서 10을 곱하고 2를 더하곘다
Y.sort() # y 값 정렬
print('X = ', X)
print('Y = ', Y)
...
def mean_square(y_pred, y):  # 예측 값(y_pred)과 실제 값(y) 간의 평균 제곱 오차 계산(손실 함수 정의한 부분)
...
for epoch in range(1, epochs + 1):
    ...
    if epoch % 50 == 0: #  50 에포크마다 현재 손실 값과 가중치,편향을 출력
